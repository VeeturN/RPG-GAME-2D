...
Skeleton2::Skeleton2(sf::Texture *texture, sf::Clock clock){
    std::mt19937 generator(std::random_device{}());
    std::uniform_int_distribution<int> distribution(5, 35);
    row=0;
    ...
    std::uniform_int_distribution<int> distributionX(2500, 3300);
    fmt::println("{}",distributionX(generator));
    std::uniform_int_distribution<int> distributionY(832, 1500);
    ...
    Skeleton2::an=animation;

};

void Skeleton2::draw(sf::RenderWindow &window) {
    window.draw(skeleton);
}
void Skeleton2::move(sf::RenderWindow &window, sf::RectangleShape shape2) {
